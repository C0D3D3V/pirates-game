...
The game is a programming battle game, it can be written in any language that supports websockets.
There is an example implementation in JavaScript. It's intended for programmers with a bit of programming experience.

The goal of the Game is to make as much gold as possible!\
Each Team can control as many ships as they can buy.
The ships are controlled over `Websockets`, the example implementation is in the `shipcontrol/client.js` file.
...
Each Team can buy as many ships as it has money for it.

## Ports
There are Ports out in the Ocean, each Port generates Gold.
As a Pirate you want to raid as many ports as possible.
...
## Overview
You can open it locally in the browser, and you should see the map.
